...
### 📊 多平台数据统计
- **饿了么固定费用**：自动统计净结算金额为 33.95元（35 - 1.05抽佣）、36.86元（38 - 1.14抽佣）以及 50.00元（51.5 - 1.5抽佣）的店铺
- **饿了么代运营回款**：每日线上回款数据统计
...
### 📅 数据筛选
- **按月份筛选**：智能月份选择器，支持查看特定月份的回款数据
  - 自动提取所有可用月份，按最新月份优先排序
  - 下拉菜单显示友好的月份格式（如"2025-10 (2025年10月)"）
  - 实时过滤图表、表格和金额卡片数据
  - 一键"清除筛选"快速返回全部数据视图
  - 使用 useMemo 优化性能，确保流畅体验

### 📈 可视化数据展示
- **概览卡片**：四个渐变卡片展示总金额和分平台金额（支持月份筛选）
- **趋势图表**：
  - 📊 每日总金额趋势图（玫瑰粉色系）
  - 👥 每日回款店铺数趋势图（青色系）
  - 平滑曲线动画效果
...
执行命令时会优先在以下固定目录中匹配每种类型最新的 Excel，并规范化到对应 `excel-input` 目录；只有对应目录没有匹配文件时，才会回退到项目根目录查找：
...
因此日常使用只需要把最新账单放在项目根目录或保留在对应 `excel-input` 目录即可，不需要每次手动挑选后再二次整理。

饿了么代运营回款（`excel-input/2` -> `elmCycleData.json`）会按最新周期账单覆盖同日期旧数据，用于同步平台回溯修正后的历史账单金额；其他数据仍按日期追加新数据。
...
1. **选择月份**：点击"筛选月份"下拉菜单，选择要查看的特定月份
2. **查看结果**：
   - 顶部金额卡片自动更新为选中月份的统计数据
   - 图表只显示该月份的趋势
   - 表格只展示该月份的每日明细
3. **清除筛选**：
   - 点击"清除筛选"按钮返回查看全部数据
   - 或在下拉菜单中选择"全部月份"
4. **实时更新**：切换月份时，所有数据视图实时同步更新

> **提示**: 月份筛选功能会从所有已上传的数据源（饿了么固定费用、饿了么代运营、美团代运营、美团线下收款）中自动提取可用月份。
...
- 统计三种固定费用档位：
  ...
  - **档位3**: 固定费用 +51.50元，抽佣 -1.50元，**净结算金额 50.00元**
- 只统计净结算金额为 33.95元、36.86元 或 50.00元 的记录
...
│   ├── components/
│   │   ├── ClientHome.tsx           # 客户端主组件(使用 localStorage + 月份筛选)
...
### ⚠️ 数据处理规则
1. **饿了么固定费用**：只统计净结算金额为 33.95元 (35-1.05)、36.86元 (38-1.14) 或 50.00元 (51.5-1.5) 的店铺记录
...
2. **数据类型**：上传前选择正确的数据类型
3. **月份筛选**：使用月份筛选器快速查看特定时间段的回款数据，方便月度对账和分析
4. **浏览器兼容**：建议使用Chrome、Firefox、Edge等现代浏览器(需支持 localStorage)
5. **数据清空**：点击右上角"清空数据"按钮可清空所有平台数据
6. **定期备份**: 由于数据存储在浏览器本地,建议定期备份 Excel 原始文件
7. **浏览器限制**: localStorage 容量限制通常为 5-10MB,对于本系统的数据量完全足够
